...
{	
  ...
  if($this->date_format['currency']=='INR' OR $this->date_format['currency']=='USD'){
   ...
   $vat_amt=$this->date_format['currency'].' '.array_sum($total_vat);
    ...
    $html.='<tr><td>Outstanding payment Customer</td> <td></td> <td>'.round($OutstandingAmt['OutstandingAmt'],2).'</td><td></td> </tr>';
    $crassetdata=array();
    foreach ($currentAsset as $key03 => $currentAssetDataVal){
    $crassetdata[]=$currentAssetDataVal['value'];
    $html.='<tr><td>'.$currentAssetDataVal['title'].'</td> <td></td> <td>'.$currentAssetDataVal['value'].'</td> <td></td></tr>';
    }

    $supplyerArr=array();
    foreach ($give_to_supplyer as $key04 => $givetoSupplyerDataval){
    $supplyerArr[]=$givetoSupplyerDataval['giveToSuppAmt'];
    $html.='<tr><td>'.$givetoSupplyerDataval['name'].'</td> <td>'.$givetoSupplyerDataval['giveToSuppAmt'].'</td><td></td> <td></td></tr>';
    }
    
    $html.='<tr><td>Current Stock</td> <td></td><td>'.round($current_stock['currentStock'],2).'</td><td></td> </tr>';
    $html.='<tr><td>Expenses,salary,fuel,phone Stock</td> <td>'.round($allexp['exp_amt'],2).'</td><td></td><td></td> </tr>';

     $html.='<tr><td>'.$primaryInvestor['name'][0].'</td> <td>'.round($primaryInvestor['amt'][0],2).'</td><td></td><td></td> </tr>';

     $html.='<tr><td>'.$secondryInvestor01['name'][0].'</td> <td>'.round($secondryInvestor01['amt'][0],2).'</td><td></td><td></td> </tr>';

     $html.='<tr><td>'.$secondryInvestor02['name'][0].'</td> <td>'.round($secondryInvestor02['amt'][0],2).'</td><td></td><td></td> </tr>';

     $html.='<tr><td>FOC Stock</td> <td>'.round($all_focAmt['foc_amt'],2).'</td><td></td><td></td> </tr>';

    $totalDebit=$allexp['exp_amt'] + $primaryInvestor['amt'][0] + $secondryInvestor01['amt'][0] + $secondryInvestor02['amt'][0]+ array_sum($supplyerArr);

    $totalcredit=$current_stock['currentStock'] + array_sum($crassetdata) + $OutstandingAmt['OutstandingAmt'];

    $balanceAmt=$totalcredit+$totalDebit;

    $html.='<tr><td>Total</td> <td>'.round($totalDebit,2).'</td> <td>'.round($totalcredit,2).'</td> <td>'.round($balanceAmt,2).'</td> </tr>';
    
    $html.='<tr><th>Balance</th><th>'.round($balanceAmt,2).'<th><td></td></tr>';
    $html.='<tr><th>Fixed Asset</th><th>'.round(array_sum($assetDataArr),2).'</th><td></td><td></td></tr>';
    
    $totalAmtBalance= $balanceAmt+array_sum($assetDataArr);

    $html.='<tr><th></th><th>'.round($totalAmtBalance,2).'</th><td></td><td></td></tr>';

    $totalAmtBalance=$totalAmtBalance-$assetCapitaldata;

    $html.='<tr><th>Capital Min(Balance)</th><th>'.round($totalAmtBalance,2).'</th><th></th><th></th></tr>';
    $html.='</table>';
    echo $html;
    }
