    ...
    public function fetchProductData()
    {
        ...
        foreach ($data as $key => $value) {
           ...
           if($this->date_format['currency']=='INR' OR $this->date_format['currency']=='USD'){
            ...
            $dId=base64_encode($value['invoiceID']);
            $urlInvoiceNo=base_url('commercialinvoice/payment_history/'.$dId);
          
            //$anker='<a href="'.$urlInvoiceNo.'" target="_blank">"'.$value['oth_invoice_no'].'"</a>';
            $anker='<a href="'.$urlInvoiceNo.'" target="_blank">'.$value['oth_invoice_no'].'</a>';
